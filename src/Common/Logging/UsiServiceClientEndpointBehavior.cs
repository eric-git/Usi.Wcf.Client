...
public class UsiServiceClientEndpointBehavior(ILogger logger) : IEndpointBehavior
{
    ...
    private UsiServiceClientMessageLoggingHandler GetHttpMessageHandler(HttpClientHandler httpClientHandler)
    {
        return new UsiServiceClientMessageLoggingHandler(httpClientHandler, logger);
    }
}