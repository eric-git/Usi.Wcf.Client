...
public class KeystoreManager(IConfiguration configuration, IMemoryCache memoryCache) : IKeystoreManager
{
    ...
    {
        var clientCredentialId = configuration[SettingsKey.KeystoreClientCredentialId] ?? throw new InvalidOperationException("Missing KeystoreClientCredentialId.");
        var certificateData = memoryCache.GetOrCreate($"KS-{clientCredentialId}", cacheEntry =>
        {
            var fileName = configuration[SettingsKey.KeystoreFileName] ?? throw new InvalidOperationException("Missing KeystoreFileName.");
            var password = configuration[SettingsKey.KeystorePassword] ?? throw new InvalidOperationException("Missing KeystorePassword.");
            XmlReaderSettings settings = new()
            {
                DtdProcessing = DtdProcessing.Prohibit,
                XmlResolver = null
            };
            using var reader = XmlReader.Create(Path.Combine(AppContext.BaseDirectory, fileName), settings);
            XPathDocument xmlDocument = new(reader);
            ...
            var x509Certificate2 = signedCms.Certificates.FirstOrDefault(c => c.SubjectName.Name.Contains(abn, StringComparison.Ordinal)) ??
